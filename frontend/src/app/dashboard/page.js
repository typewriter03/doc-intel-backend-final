...
export default function DashboardOverview() {
  ...
  const stats = [
    { name: 'Total Workflows', value: workflows.length.toString(), icon: Zap, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    { name: 'Processed Docs', value: analytics.total_docs.toString(), icon: FileText, color: 'text-green-500', bg: 'bg-green-500/10' },
  ...
  useEffect(() => {
    async function fetchData() {
      if (user) {
        setLoadingData(true);
        try {
          const analyticsData = await api.getAnalytics(7);
